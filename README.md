...
- 新增自动监看：使用 `watchdog` 文件事件监听图片目录，默认 30 秒防抖后自动扫描、生成缩略图、同步元数据；默认不会轮询扫盘。
- 新增前端实时刷新：后端通过 SSE 推送自动扫描和任务事件；SSE 断开时不会启动后台定时扫描，重新打开页面后会重新建立连接。
- 新增增量扫描和增量元数据同步：只处理新文件、stat 变化的图片或缺失元数据的作品。
- 新增轻量预热缓存：画廊按当前排序和筛选结果，预热切换显示模式所需的少量图片；关闭显示窗口一段时间后自动释放图片资源。
- 新增去重中心：按同一 Pixiv 作品的同一页筛选候选，再计算 SHA-256 确认重复；支持自动勾选较晚文件、实际删除源文件和去重日志。
- 新增怀疑图片列表：同一作品同一页的候选文件如果 hash 不一致，会进入怀疑列表，适合检查不同分辨率或不同编码的版本；支持自动勾选较低分辨率文件。
- 新增整理文件：支持多源目录、Shaft 风格路径模板、自定义模板、复制/移动/软链接/硬链接、可选自动重命名和本次整理撤销。
- 新增 R18 / AI 识别：R18 使用统一标签规则，AI 优先使用 Pixiv 官方 `ai_type` 字段，并同步到筛选、角标和整理路径。
- 新增设置页任务控制、最近操作日志、全量扫描、重建元数据、查看器文件夹入口和原图自动显示等选项。
- 详情页底部操作区固定显示原图、作品切换、收藏、打开文件夹和删除操作；上方详情信息独立滚动，避免操作按钮随内容滚出视口。

**1.2.1**

**1. 新功能：收藏订阅**
- 订阅任意 Pixiv 用户的**收藏列表**：输入用户 ID（用户主页 URL 中的数字），首次订阅会自动下载该用户公开收藏的全部作品；之后每次点「订阅更新」只**增量检查**并下载上次检查后新收藏的作品，无需重新扫描整个列表。
- **全自动处理**：一次检查任务内完成「下载新收藏 → 扫描入库」，扫描发现作品后会并行生成缩略图和同步元数据，结束后画廊直接可看，全程无需手动操作。
...
- **本地扫描入库**：递归扫描本地 Pixiv 下载目录，从文件名解析作品中对应的 Pixiv ID（兼容 `12345678_p0` 等多页命名），按文件名页码排序并写入 SQLite 索引（`archive.db`）。
...
- **AI / R18 分类**：同步 Pixiv 官方 AI 状态，`ai_type=2` 归类为 AI，`ai_type=1` 归类为 Human；R18 使用精确标签匹配，不把 `R-18G` 当作 R18。
- **整理文件**：按 Shaft 风格或自定义规则组织目录，可选择复制、移动、软链接或硬链接；文件名默认保持旧版行为，也可单独启用重命名模板和撤销。整理设置会保存到 `.env`。
...
左下角任务状态栏显示当前任务、进度、暂停/继续和取消操作。点击任务状态可进入设置页的操作日志窗口。扫描、缩略图、元数据同步、订阅更新、去重和整理文件均会写入结构化日志；容器环境可直接使用：
...
升级时请保留宿主机映射的 `/app/data` 目录，其中包含数据库、元数据和缩略图。不要把 `.env`、`data/`、原始图片目录或运行日志提交到 GitHub。
...
4. 若不存在 `.env`，自动从 `.env.example` 复制生成。
...
| `PA_METADATA_COMPACT` | 是否紧凑保存元数据 JSON，默认 `1` |
| `AUTO_FAIL_PAUSE_ENABLED` / `AUTO_FAIL_PAUSE_THRESHOLD` | 连续失败自动暂停及其阈值 |
| `PA_SHOW_FOLDER_BTN` | 是否显示查看器“在文件夹中打开”入口 |
| `PA_VIEWER_AUTO_ORIGINAL` | 打开详情页时是否自动切换原图 |
| `PA_SIDEBAR_HOVER_EXPAND` | 是否允许鼠标悬停展开侧边栏 |
| `ORGANIZE_*` | 整理文件源目录、输出目录、路径模板、重命名和整理方式；建议在设置页修改 |
...
更多扫描机制、任务并行、去重、整理文件和 Docker 排查说明见
[`docs/FEATURES_AND_ARCHITECTURE.md`](docs/FEATURES_AND_ARCHITECTURE.md)。
