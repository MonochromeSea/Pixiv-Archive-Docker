...
**1. 前端界面全面重设计**
- 布局由「顶部工具栏」改为「左侧边栏 + 内容区」：侧栏集中导航（画廊 / 作者列表 / 标签列表 / 收藏夹 / 订阅）、操作（同步 / 扫描 / 按作者下载 / 订阅更新）与底部（主题 / 设置 / 作品总数）。
- 侧栏支持**收起**：收起后仅显示图标（品牌缩为 "PA"），悬停时临时浮层展开完整菜单，状态跨页面记忆；窄屏（<900px）自动收为图标栏，悬停图标记浮出文字提示。
- 全站 emoji 图标（⚙ ☀️ 📁 ★ 等）替换为统一的**线条风格 SVG 图标体系**（23 个内置图标，无外部依赖）。
- **亮 / 暗主题完整重做**：更有层次的深色背景与组件质感（Linear/macOS 风格），亮色主题同步重调；图片查看器不再固定深色，完整跟随主题。
- 弹窗（毛玻璃遮罩 + 面板动画）、Toast、右键菜单、任务进度窗、批量操作条、分页、空状态、设置面板全部重绘。
- 交互修复与优化：瓷砖勾选框整个可见区域都可点击（原来只有 13px 复选框本体可点）；`ESC` 可逐层关闭所有弹窗；搜索结果现在显示「搜索：关键词」状态条并可一键清除；切换视图自动回到顶部；关闭查看器时清理残留右键菜单；任务结果窗标题修正（此前手动下载完成误显示为"同步元数据"）。

**2. 画师订阅（新功能）**
- 新增「订阅」视图：订阅卡片显示画师名、本地作品数、当前游标、上次检查时间与结果，支持单独检查 / 退订 / 自动下载开关。
- **增量检查用 PID 游标**：每个订阅记录已下载的最大 Pixiv ID，检查时只拉画师作品列表的最新一页（Pixiv 按发布时间倒序返回），遇到 ≤ 游标的作品即停止翻页——不逐个比对历史 ID，对高产画师也几乎零开销。
- **一键全自动流水线**：检查发现新作品后，同一任务内依次完成 下载 → 扫描入库 → 生成缩略图 → 同步新作品元数据，完成后画廊直接可看，无需任何手动步骤。
- **从账号导入**：可一键把 Pixiv 账号的「关注画师」或「收藏作品对应的画师」批量导入为订阅（自动去重、已订阅的跳过，分页拉取受限速保护）。
- 订阅前**预览**：添加画师时可先「查询」，显示画师名与按最近内容估算的新作品数量，再决定是否订阅；添加时可选「立即检查并下载」。
- 下载失败的作品不推进游标，下次检查自动重试；单订阅与全量检查都走全局任务锁（与扫描 / 同步 / 手动下载互斥排队），进度窗实时显示且可随时停止。
- 命名、目录、去重规则与「按作者下载」完全一致（共享同一套下载代码）。

**3. 下载与网络**
- **第三方图片镜像（可选）**：设置「网络设置」新增「用第三方图站接管图片下载」开关与镜像域名输入。开启后「按作者下载 / 订阅下载」的图片字节与作者头像改由镜像域名获取（保留原路径，域名自动规范化），作品列表与认证仍走 Pixiv 官方 API；头像遇镜像不支持时自动回退官方地址。
- 认证失败提示现在透传底层原因，遇到 TLS 被掐断时明确提示"代理未把 oauth.secure.pixiv.net 走节点，请检查代理分流规则"，不再误导向 refresh token。

**4. 稳定性与正确性修复**
- SQLite 连接增加 `busy_timeout=30`：同步 / 扫描任务持有写事务期间，界面上的收藏、删除等操作不再随机报 "database is locked"。
- 任务启动原子化：「检查占用 → 登记新任务」合并进同一把锁，杜绝连点按钮导致扫描与同步并发运行、互相覆盖状态的竞态。
- 下载改为 `.part` 临时文件 + 成功后原子改名：网络中断 / 超时不再留下半截文件被误判为"已存在"进而入库坏图。
- `/api/artworks` 移除了重复执行两遍的 COUNT 查询（首页最热接口的复制粘贴残留）。
- 移除设置接口中重复的同步间隔校验块。
...
- **画师订阅**：订阅画师后按 PID 游标增量检查新作品，一键完成 下载 → 入库 → 缩略图 → 元数据同步 全流程；支持从账号关注/收藏批量导入订阅（v1.2 新增）。
